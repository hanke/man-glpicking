...
\mydiffX{with a}{by subclassing QT's} \lstinline|QGLWidget|.
%
...
\mydiffX{as}{has} to be activated (line 50, listing \ref{lst:ctor}).
%
%%%\ifthenelse {\boolean{UseOrig}}
%%%{
%%%\begin{lstlisting}
...
class \lstinline|ExampleRenderThread| \mydiffX{(see XXX)}{, cf. Appendix \ref{sec:app-d},} which implements the
GL-picking.
%
...
130 }
\end{lstlisting}
\caption{Pass resize events \mydiffX{onto}{on to} the rendering thread}
\label{lst:resize}
\end{figure}
%%%}
...
thread.
%
Its first action is to lock the render mutex of the \mydiffX{GL}{OpenGL} widget and to make the
...
%
As long as \lstinline|render_flag| is set to \mydiffX{true}{\lstinline[keywordstyle=\color{black}]|true|}, the rendering is executed.
%
...
the \mydiffX{}{\lstinline|run()|} function).
%
...
%
Once a surface has been \mydiffX{clicked on}{double-clicked}, its ``name" can be found at the fourth
...
of the \mydiffX{clicked}{selected} surface (listing \ref{lst:picking}).
%
\begin{figure}[h]
\begin{lstlisting}[basicstyle=\scriptsize]
206   glRenderMode(GL_SELECT);
...
We have shown how to implement \mydiffX{multi-threaded OpenGL with picking capabilities}{OpenGL with picking capabilities in a multi-threaded fashion}
