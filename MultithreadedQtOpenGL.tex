\documentclass[jou,noapacite]{apa}
\pdfoutput=1
...
%
%\usepackage{color}
%\newcommand{\mydiffX}[2]{{\color{blue}#1} $\to$ {\color{red}#2}}
%\newcommand{\mydiff}[2]{{}{\color{red}#2}}
%\newcommand{\mydiffX}[2]{{}{#2}}
%
...
%
%
\rightheader{Multi-threaded OpenGL with Qt}
\leftheader{S. C. Kramer and M. Hanke}
%
%\usepackage{ifthen}
%
%
...
% }
% }
\twoauthors{Stephan~C.~Kramer}{Michael~Hanke}
\twoaffiliations{%
Max-Planck Institute for Biophysical Chemistry;
Georg-August-University, G\"ottingen, Germany
}{%
Institute of Psychology II, Otto-von-Guericke-University, Magdeburg, Germany\\
...
\abstract{We discuss how to realize multi-threaded
...
\volume{INSERT ID}
\ccoppy{This work is licensed under a Creative Commons Attribution-ShareAlike 3.0 License}
...
\comment{SCK: Actually, it does. See the``tackle'' comment below. Nevertheless, the description of the problem is kind of indirect. This clearly can be improved. }
...
\comment{SCK: Lesson 32 is neither a good are a bad example. I suppose it is one of better known examples for OpenGL's picking capabilities. Since it is neither multi-threaded nor is QT employed to implement it, Lesson 32 misses two of our three goals and hence just can serve for illustrating ``picking'' in action. That's it. }
%
...
Dimension'' \cite{glimpse3d}. Mandelbrot is a fully functional, non-OpenGL
example and is available in the collection of Qt demo programs.

When it comes to combining GL-picking and Qt, finding a template program is not
quite as easy.
%
...
\comment{SCK: The two sentences are relevant. They exactly state the problem: There are programs exemplifying OpenGL's picking capabilities and there are plenty of QT programs. Where do we get an example of OpenGL picking implemented with QT's OpenGL wrapper/convenience classes? Answer: from Blanchette's book, i.e. Ref. [3]. 

The solution proposed by Michael on his webpage simply boils down to take the picking+QT part from Blanchette and Summerfield and to add the multi-threaded rendering from  ``Glimpsing the Third
Dimension''.  }
%
\comment{SCK: This is the problem we tackle:}
{\sf But multi-threaded OpenGL rendering with picking capabilities seems to be gap
...
\comment{SCK:  ``concise examples with
high quality documentation'' are always a scarce resource. One of the few projects dealing with this point seriously which I know of is the finite-element library deal.II (www.dealii.org). Would be fun to cite them in this context.

``Easy thing'' just means: ``combine the two example programs and do not re-invent the wheel.'' At least to my knowledge Michael's example is the only one on the web dealing with the problem ``QT+multi-threading+OpenGL-picking''. }
...
%
While there are no special headers to be included in case of\todo{ Windows} or
{\sf Mac OS X},\todo{ Linux} requires the \lstinline|XInitThreads()| function to
be called at the very beginning, even before the \lstinline|main()| function, at
the start-up of the program in order to make calls to the \todo{ XLib} library
thread-safe.
%
...
%
In particular, in case of Linux, we need the main header for the\todo{ XLib}
...
%
% {where are the security measures for faceatposition?}
...
\begin{figure*}[bp]
\begin{center}
  \includegraphics[width=.95\linewidth  %, clip, trim = 50mm 74mm 100mm 36mm
  ]
  {threadedcube_screenshot.png}
 %
 %\vspace{-.2cm}
  \centeredcaption{.95\linewidth}{Screenshot of application with active context menu after picking the
  left face.
}
...
\label{sec:app-b}
\lstinputlisting{src/exampleglwidget.h}
\newpage
%
\section{Source code \texttt{exampleglwidget.cpp}}
\label{sec:app-c}
\lstinputlisting{src/exampleglwidget.cpp}
\newpage
%
\section{Source code \texttt{examplerenderthread.h}}
\label{sec:app-d}
\lstinputlisting{src/examplerenderthread.h}
\newpage
%
\section{Source code \texttt{examplerenderthread.cpp}}
\label{sec:app-e}
\lstinputlisting{src/examplerenderthread.cpp}
\newpage
